...
function App() {
  ...
  const pendingCount = draft.pendingChanges.reduce((count, set) => count + set.operations.filter(({ status }) => status === 'pending').length, 0)
  ...
  const focusValidationSection = (section?: string) => {
    const targets: Record<string, { workspace: WorkspaceId; selector: string }> = {
      ...
      'resource-plan': { workspace: 'setup', selector: '.resource-editor' },
      'mission-content': { workspace: 'mission', selector: '#mission-lesson-identity' },
      'success-criteria': { workspace: 'mission', selector: '#success-criterion-1' },
      'assessment-evidence': { workspace: 'mission', selector: '#assessment-evidence-1' },
      'cycle-durations': { workspace: 'mission', selector: '#duration-plan' },
      'support-instructions': { workspace: 'adapt', selector: '#support-instructions' },
      'extension-instructions': { workspace: 'adapt', selector: '#extension-instructions' },
      'review-changes': { workspace: 'review', selector: '.change-review' },
    }
    ...
    requestAnimationFrame(() => requestAnimationFrame(() => {
      ...
      if (!element) return
      if (!element.matches('input, textarea, button, select, [tabindex]')) element.tabIndex = -1
      element.focus()
  ...
  return <div className="app-shell">
    <header className="product-header">
      ...
      <div className="header-state" aria-label="Lesson status"><WebMcpStatusIndicator status={webMcpStatus} /><ProvenanceMarker type="teacher-approval-required" /></div>
    </header>
    <WorkspaceTabs active={activeWorkspace} onNavigate={setActiveWorkspace} pendingCount={pendingCount} validationStatus={validationStatus} />
    <main className="workspace">
      ...
      <section className="workspace-panel" hidden={activeWorkspace !== 'setup'} aria-labelledby="setup-workspace-title">
        ...
        <section className="setup-controls resource-editor" aria-labelledby="resources-title"><h3 id="resources-title">Tangible resources</h3>{resourceFields.map(({ key, label, max }) => <div className="stepper" key={key}><span id={`${key}-label`}>{label}</span><button type="button" aria-label={`Decrease ${label}`} disabled={resources[key] === 0} onClick={() => updateResource(key, resources[key] - 1)}>−</button><output aria-labelledby={`${key}-label`}>{resources[key]}</output><button type="button" aria-label={`Increase ${label}`} disabled={resources[key] === max} onClick={() => updateResource(key, resources[key] + 1)}>+</button></div>)}<label className="check-label tile-only-control"><input type="checkbox" checked={resources.allowTileOnlyGroups} onChange={(event) => updateResource('allowTileOnlyGroups', event.target.checked)} />Allow tile-only groups without a robot</label></section>
        </div>
      </section>
      <section className="workspace-panel" hidden={activeWorkspace !== 'mission'} aria-labelledby="mission-workspace-title">
        <div className="workspace-heading"><div><p className="eyebrow">Accepted lesson canvas</p><h2 id="mission-workspace-title">Mission</h2></div><ProvenanceMarker type={sectionProvenance('lesson-identity')} /></div>
        <section className="mission-controls panel" aria-labelledby="mission-controls-title"><h3 id="mission-controls-title">Mission identity</h3><label>Mission theme<input value={mission.theme} maxLength={160} placeholder="e.g. A lost story path" onChange={(event) => updateMission({ theme: event.target.value })} /></label><label>Challenge level<select value={mission.challengeLevel ?? ''} onChange={(event) => updateMission({ challengeLevel: event.target.value ? event.target.value as MissionContent['challengeLevel'] : null })}><option value="">Choose a level</option><option value="introductory">Introductory</option><option value="core">Core</option><option value="stretch">Stretch</option></select></label></section>
        <section className="lesson-canvas" aria-labelledby="lesson-title">
        <div className="canvas-intro"><div><p className="eyebrow">Fictional lesson draft</p><h2 id="lesson-title">{draft.title}</h2><p className="lesson-summary">Teacher edits and validation are saved locally in this browser.</p></div><span className="draft-badge">{draft.status === 'ready' ? 'Ready for teacher review' : draft.status}</span></div>
        ...
        <section className="mission-section" aria-labelledby="mission-section-title"><div className="section-heading"><div><p className="eyebrow">Structured lesson canvas</p><h3 id="mission-section-title">Mission content</h3></div><ProvenanceMarker type={sectionProvenance('lesson-identity')} /></div>
      ...
      <section className="workspace-panel" hidden={activeWorkspace !== 'adapt'} aria-labelledby="adapt-workspace-title">
        <div className="workspace-heading"><div><p className="eyebrow">Teacher decisions</p><h2 id="adapt-workspace-title">Adapt</h2></div><ProvenanceMarker type={sectionProvenance('learner-support')} /></div>
        <section className="setup-controls adaptation-controls" aria-labelledby="adapt-controls-title" aria-describedby={!missionExists ? 'adapt-unavailable' : undefined}>
          <h3 id="adapt-controls-title">Adapt learners</h3>
...
function ValidationPanel({ draft, webMcpStatus, onAcknowledge, onEditSection }: { draft: LessonDraft; webMcpStatus: WebMcpStatus; onAcknowledge: (id: string) => void; onEditSection: (section?: string) => void }) {
